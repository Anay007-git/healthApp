...
import { AIStructuredResponse, Source, ClaimAnalysisResult } from "@civiclens/types";
import { fetchLiveKnowledge, LiveNewsArticle, LiveKnowledgeResult } from "./factcheck/live-knowledge";
import { runFactCheck } from "./factcheck/factcheck-engine";

export { fetchLiveKnowledge, runFactCheck };
export type { LiveNewsArticle, LiveKnowledgeResult };
...
- **Public Responsiveness & Crisis Management (15% Weight)**: **${breakdown.publicResponsiveness}/100**`,
    ...
    if (results.schemes.length === 0 && results.states.length === 0 && results.cag.length === 0) {
      try {
        ...
        if (liveKnowledge && liveKnowledge.extract && liveKnowledge.extract.length > 15) {
          const liveSources: Source[] = [
            {
              ...
              publisher: "Discovery registry (not independent verification)",
          ...
          return {
            answer: `### 🌐 Live Discovery Results for "${userQuestion}"\n\n${liveKnowledge.extract}\n\n- **Status**: These hits are **discovery/context only**. News coverage or encyclopedia snippets do not prove a claim true.\n- **Citation**: [${liveKnowledge.sourceLabel}](${liveKnowledge.sourceUrl})\n- **Next step**: Use TruthCheck™ to run evidence-first verification of a specific factual claim.`,
            metrics: [
              { label: "Search Status", value: liveKnowledge.channel === "GOOGLE_NEWS" ? "News discovery" : "Background context" },
              { label: "Proof value", value: "Not verification" },
              { label: "Domain Category", value: liveKnowledge.category },
              { label: "Channel", value: liveKnowledge.channel },
            ],
            sources: liveSources,
            confidence: "LOW",
            methodology: "Google News, Wikipedia, and DuckDuckGo are queried as discovery/context. They are not treated as proof.",
          };
        }
      } catch (err) {
  ...
  async analyzeMisinformation(rawClaimText: string): Promise<ClaimAnalysisResult> {
    return runFactCheck(rawClaimText);
  }
